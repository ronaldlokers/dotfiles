...
Both the plugin and its place on the bar arrive with an apply, as does
`lokilabs.workspace` — the workspace-indicator clone that groups spaces into
per-monitor colour bands, which replaces `omarchy.workspaces` rather than
joining it. `shell.json`
...
| `dot_claude/` | Claude Code: global `CLAUDE.md`, statusline, `rtk-rewrite` hook, `modify_settings.json` (which plugins and marketplaces are enabled) |
...
| `dot_local/bin/` | own scripts: `repos-sync`, `devcontainer-init`, `dotfiles-update-check`, `proton-ssh-load`, `hypr-center-master` (SUPER + M, lays the active workspace out 25/50/25), and `devpod` (wrapper for the binary in `~/.local/libexec`) |
